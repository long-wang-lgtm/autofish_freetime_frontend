...
export function useDraftsData({ page, pageSize, search, status, toUid }: UseDraftsDataParams) {
  ...
  const queryKey = useMemo<unknown[]>(
    () => ['batch-publish', 'materials', 'drafts', { page, pageSize, search, status, toUid }],
    [page, pageSize, search, status, toUid],
  )
