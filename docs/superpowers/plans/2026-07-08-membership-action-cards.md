# MembershipActionSheet 卡片化改造 实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 MembershipActionSheet 的选择交互从 `<select>` 改为信息卡片，用户可看到会员等级/充值档位的完整信息后再做选择。

**Architecture:** 仅重写 `app/admin/users/MembershipActionSheet.tsx`。新增 PlanCard/StoneCard 私有组件 + 价格计算函数；保留现有状态管理和提交逻辑；重写 JSX 渲染层。

**Tech Stack:** React + Tailwind CSS v3, lucide-react, SlidePanel, sonner toast

---

### Task 1: 重写 MembershipActionSheet.tsx

**Files:**
- Modify: `app/admin/users/MembershipActionSheet.tsx`（全量重写）

> 说明：当前文件 317 行。保留状态管理、副作用、提交逻辑不变。新增 PlanCard/StoneCard 私有组件（文件顶部）、价格计算函数、重写 JSX。

- [ ] **Step 1: 清空文件并写入完整新代码**

用以下内容**完全替换** `app/admin/users/MembershipActionSheet.tsx`：
...
export function MembershipActionSheet({ open, onClose, action, user, onSuccess }: Props) {
  ...
  // ---- 获取指定等级的 plan 数据 ----
  const getPlanByTier = (tier: number) => plans.find((p) => p.tier === tier)
...
预期：构建成功，`/admin/users` 路由编译通过。

- [ ] **Step 4: Commit**

```bash
git add app/admin/users/MembershipActionSheet.tsx
git commit -m "refactor: replace select dropdowns with PlanCard/StoneCard selection cards and auto price calculation"
```
...
- [ ] **Step 2: 功能自查清单**
  - [ ] 升级：当前等级卡片 disabled 灰色展示 → 可选等级卡片列表（过滤 tier > currentTier） → 点击选中后金额自动计算 → 计算过程文字显示
  - [ ] 降级：当前等级卡片 disabled 展示 → 可选等级卡片列表（过滤 tier < currentTier） → 点击选中金额不自动计算
  - [ ] 续费：当前等级卡片 disabled 展示 → 月数输入 → 金额实时自动计算（月数 × 单价）
  - [ ] 充值：当前余额展示 → 档位卡片 grid-cols-2 → 点击选中
  - [ ] 所有操作：备注字段、提交按钮、toast 反馈、onSuccess 回调
  - [ ] 边界：无更高等级时显示 EmptyState → 已是最高等级
  - [ ] 边界：无更低等级时显示 EmptyState → 已是最低等级
  - [ ] 边界：定价列表为空时显示 EmptyState → 暂无可选定价
