...
| 模块小标题 | `text-[10px]` | `text-[11px]` | ProductDiagnosticDrawer（趋势诊断）, WindowCompareCards（核心指标）, StabilityPanel, AnomalyBanner（异常预警）, GrowthPricePanel（💰商业表现/📈流量周期/📊采集质量） |
...
| 图表空态占位 | `h-40` | `h-48` | 三个图表（无数据时占位 div 高度同步） |
| 图表容器高度 | `h-40` | `h-48` | 三个图表 |
| StabilityPanel 底部注释 | `text-[10px]` | `text-[11px]` | StabilityPanel（"基于窗口内 N 个数据点"行） |
...
移除独立的"趋势诊断"小标题（图表自带 HTML 标题已足够），同时移除 Part 1 与 Part 2 之间的 `<hr>` 分隔线（grid 的 gap 天然形成视觉分区），将 Part 1 + Part 2 合并为一个 grid：
...
左半列宽约 40vw（≈768px @ 1920px 屏宽），矩阵表格 8 列。`text-[11px]` 字号下每列均宽约 95px，delta 列标签 "vs D1"/"vs D3" 仅 4 字符无需缩短，"D1→D7 总Δ" 列标签 9 字符可完整显示。表格保留 overflow-x-auto 兜底小屏。
...
`seriesName` 实际收到的是一个数字（dataIndex）。number 原型上不存在 `includes` 方法，`?.` 可选链仅在 receiver 为 `null`/`undefined` 时短路——number 不是 null/undefined，因此 `(42).includes('率')` 会抛 `TypeError`。ECharts 内部 catch 此异常后退回默认格式化，表现为所有值都走 `toLocaleString('zh-CN')` 路径。
...
| 文件 | 改动要点 | 行数估算 |
|------|---------|---------|
| `ProductDiagnosticDrawer.tsx` | width 80vw, 2×2网格替代纵向堆叠, 移除"趋势诊断"小标题和对应hr, 移动端条件布局, 字体批量 text-[10px]→text-[11px], 上架天数 Math.round + datetime | +30 / -22 |
| `CumulativeGrowthChart.tsx` | 双Y轴（左浏览右想要+收藏）, 图例标注轴归属, grid right 16→56, 容器+空态 h-40→h-48, X轴 fontSize 9→10, 图例 text-[9px]→text-[10px] | +26 / -16 |
| `IntentConversionChart.tsx` | valueFormatter 修正（数据层精度控制+通用格式化）, 参考线颜色改为图例同色（蓝/青）, 容器+空态 h-40→h-48, X轴 fontSize 9→10, 图例 text-[9px]→text-[10px] | +21 / -16 |
| `TrafficActionChart.tsx` | valueFormatter 修正, 容器+空态 h-40→h-48, X轴 fontSize 9→10, 图例 text-[9px]→text-[10px] | +11 / -11 |
...
| `StabilityPanel.tsx` | 小标题+底部注释 text-[10px]→text-[11px] | +2 / -2 |
