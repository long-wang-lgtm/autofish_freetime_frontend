...
| # | 冲突 | 处理方式 |
|---|------|---------|
| 1 | `CategoryCard.tsx` 内部硬编码跳转 `/dashboard/selection/:id`，新建页面后该路由生效 | 不修改 CategoryCard，自然兼容 |
...
│   ├── ProductHistoryPanel.tsx    # 🆕 右侧历史详情面板（主容器）
│   └── TrendChart.tsx             # 🆕 可交互完整趋势图（面板内用）
│
├── merchant/                      # 商户监控 Tab
...
**展示形式：** 右侧固定面板（非抽屉 overlay），与表格并排。表格宽度自适应收缩，面板占约 40%。
...
| # | 冲突 | 处理方式 |
|---|------|---------|
| 2 | 新增"近期趋势"列会使表格总列数变为 16，可能影响横向滚动和列宽分配 | 不删改现有列，新列给固定宽度 100px，表格自然增加横向滚动 |
| 3 | 行点击选中与监控状态 badge 点击事件可能冒泡冲突 | badge 的 onClick 使用 `e.stopPropagation()` 阻止冒泡；操作按钮同样处理 |
| 4 | 历史面板展开时表格宽度收缩，可能影响现有列的可读性 | 面板给固定宽度 420px，表格 flex-1 自适应；当窗口宽度 < 1200px 时面板自动转为 overlay 抽屉模式 |
...
app/dashboard/selection/page.tsx         # 🆕 独立选品监控页
  ...
  │   └── ProductHistoryPanel (product/)  # 🆕 右侧历史面板
