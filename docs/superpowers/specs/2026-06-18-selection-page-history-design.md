...
| # | 冲突 | 处理方式 |
|---|------|---------|
| 1 | `CategoryCard.tsx` 硬编码 `<Link href={/dashboard/selection/${category.id}}>`，与 `/dashboard/selection` 静态页共享路径前缀，在 Next.js 中冲突（且违反 CLAUDE.md "严禁动态路由" 约束） | CategoryCard 当前 orphaned（未接入页面），冲突是潜伏的。接入 keyword Tab 时将其链路改为查询参数 `/dashboard/selection?category=id`，或改为 `/dashboard/selection/keyword?category=id` |
...
`components/selection/` 共 19 个文件，其中 6 个被 `publish/page.tsx` 或同目录文件引用，13 个 orphaned（未被任何页面导入）。
