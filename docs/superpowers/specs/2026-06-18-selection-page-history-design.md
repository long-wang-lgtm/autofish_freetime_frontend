...
- 顶层 `SelectionTabBar` 切换 keyword / product / merchant，默认选中 `keyword`（与现有一致）
...
> CategoryCard 原本硬编码动态路由 `/dashboard/selection/${category.id}`，与新建静态页冲突。该文件当前 orphaned（未被任何页面引用），**重组时直接删除**，冲突消除。相关配套文件 CategoryCardGrid / CategoryTypeTabs 同样 orphaned，一并删除。（保留页面状态用查询参数，符合 CLAUDE.md 约束）
...
`components/selection/` 共 19 个文件，其中 6 个被引用，10 个 orphaned（重组后复用），3 个 orphaned 且含致命问题（CategoryCard / CategoryCardGrid / CategoryTypeTabs，动态路由违规），**重组时删除**。最终保留 16 个文件。
...
│   └── SettingsDrawer.tsx         # 🆕 设置面板（容纳 config/ 组件，基于 Sheet）
│
├── keyword/                       # 关键词采集 Tab
...
| 归入 | 文件 | 理由 |
|------|------|------|
| `shared/` | TabBar, ViewToggle, DateListSidebar, HeatmapCalendar, SettingsDrawer | 跨 Tab 或页面级使用，无业务耦合 |
| `keyword/` | KeywordsConfig | 对标关键词配置 |
| `keyword/` | ReportCard, ReportControlBar, ReportSubTabs | 日报和选品汇总是关键词采集链路的输出产物 |
| `product/` | ProductMonitorTab + 3 个新组件（MiniTrendChart, ProductHistoryDrawer, TrendChart） | 商品监控，自成一域 |
| `merchant/` | MerchantMonitorTab | 商户监控，自成一域 |
| `config/` | AccountsConfig, AIConfig, CollectionConfig | 页面级配置面板，不属于任一 Tab |
| *(删除)* | CategoryCard, CategoryCardGrid, CategoryTypeTabs | orphaned + 动态路由违规，重组时删除 |
...
- 同目录交叉引用（`KeywordCollectionTab` → `NewKeywordModal` / `VerticalTimeline`）— 更新为相对路径
- 其余组件对 `@/stores/selection.store` 或 `@/lib/api/selection` 的引用 — 路径不变，仅组件自身的 `@/components/selection/` 交叉引用需更新
...
- `selectedProductId` 状态放在 `ProductMonitorTab` 内部（Tab 级），切换 Tab 后回来不保持选中（自然重置）
- 默认无选中，Sheet 关闭
...
| 场景 | 展示 |
|------|------|
| 未选中任何商品 | Sheet 不渲染（`open={false}`），表格无选中高亮 |
| 选中商品但无历史数据 | 抽屉内显示"该商品暂无历史采集数据"，灰色居中 |
| 数据加载中 | 抽屉内各区块显示骨架屏 |
| API 请求失败（网络错误/500） | 抽屉内显示错误提示 + 重试按钮 |
...
**方案：** 选品监控页面顶部工具栏（TabBar 同行右侧）放置"⚙ 设置"按钮，点击弹出 `SettingsDrawer`（基于 Sheet），内含三个配置组件垂直排列。

**注意：** 三个配置组件当前使用本地 `useState` + hardcoded 数据，无 API 调用。放入抽屉直接可用，但 `AccountsConfig` 和 `AIConfig` 自带 `bg-white rounded-xl p-5` 卡片包裹，在抽屉内可能视觉冗余（card-in-card），实现时去掉外层卡片样式即可。

**图表库选择：** 历史抽屉中的双折线图（solid/dashed lines、tooltip、双 Y 轴）需要图表库支持。选择留到实现阶段，优先考虑已在项目中使用的库（如有），否则按最小依赖原则选取（如 recharts）。
