...
**日聚合曲线（近 30 日趋势）不放图例、末段用虚线 + 空心点**：图里只有合计曲线（销量 / 销售额各一条，指标色），账号取舍交给上方筛选栏——全选看总量、单选某个账号看它自己（2026-09-21 用户拍板简化）。末段（今日）还没走完，单独画成虚线 + 空心末点，免得把半天的合计读成掉头：一条指标拆三个系列（实线到昨天 / 末段虚线 / 末点空心圈）。

**数据标签只写数值本身**（不带「最高」这类文字提示），取点靠 `labelIndices`（关键点 + 等距中间点）控密度：

- **每日成交趋势**（30 天单曲线）：关键点必标——首点、最高点、昨天（倒数第二个点）、今天（末点），再按密度补等距中间点（30 天里最多 ~6 个；与关键点相邻的中间点一律丢掉，否则两字叠一起）。共 ~8 个标签/子图。
- **分时段成交**（24 小时 × 3 条曲线）：三条曲线同一个小时的值常常一模一样（实测 00 点三天都是 4），每个点都标会在同一处叠三个字 → **历史灰线只标峰值**（正好把三天的峰值摆在一起比），今日（主角）给首点 00 / 峰值 / 最后一个有数据的小时 + 等距中间点。
- 标签位置：**首点贴左边线必须用 `position: 'right'`**（'top' 会压住值轴刻度）；昨天用 `'bottom'`（与末点只隔一个点，一上一下才不叠字）；历史日按天错开 bottom/top。

实现用 `markPoint`（**需在 `components/ui/chart/echarts.ts` 注册 `MarkPointComponent`**，`markPoint.data[i].name` 是类型必填项，给日期/小时即可）；`labelIndices` / `seriesMaxIndex` 在 `lib/utils/chart-axis.ts`。

两个实测踩过的坑：
- 逐项 `data[i].symbol` 会被 series 级 `symbol: 'none'` 压掉 —— symbol 要放在**单独的点系列**上给；
- 别用 `emptyCircle`：它的填充被强制为白、描边吃 `itemStyle.color`，给白描边就等于隐形 —— 用 `symbol: 'circle'` + `itemStyle: { color: '#fff', borderColor: <指标色>, borderWidth: 2 }`；
- 虚线用 `lineStyle.type: [4, 3]` 自定短划：末段只有一格宽（~20px），默认的长 dash 会看成一截实线。
