...
**筛选栏常驻吸顶**（原则 3：用户在任何滚动深度都可能要切筛选）：外层包一层 `sticky top-0 z-20 bg-gray-50 py-1`，背景铺成页面底色（`bg-gray-50`）——滚上去的内容会被这层底色盖住，不会从卡片圆角/缝隙里透出来。项目内先例：仪表盘账号筛选栏、`TabBar`（`sticky top-0 z-10 bg-gray-50`）。滚动容器是 `<main class="overflow-auto">`，sticky 生效范围内即可，不需要监听滚动。
